...
#define CCSP_DATAMODEL_XML_FILE "MotionDetection.xml"
...
PCOMPONENT_COMMON_MOTIONDETECTION g_pComponent_COMMON_MotionDetection  = NULL;
    ...
    )
{
    ...
    g_pComponent_COMMON_MotionDetection = (PCOMPONENT_COMMON_MOTIONDETECTION)AnscAllocateMemory(sizeof(COMPONENT_COMMON_MOTIONDETECTION));
    if (!g_pComponent_COMMON_MotionDetection)
    {
        return ANSC_STATUS_RESOURCES;
    }

    ComponentCommonDmInit( g_pComponent_COMMON_MotionDetection);

    g_pComponent_COMMON_MotionDetection->Name = AnscCloneString(CCSP_COMPONENT_NAME_MOTIONDETECTION);
    g_pComponent_COMMON_MotionDetection->Version  = 1;
    g_pComponent_COMMON_MotionDetection->Author   = AnscCloneString("GlobalLogic");
    ...
    )
{
    ...
    char                            CrName[256];
     ...
     g_pComponent_COMMON_MotionDetection->Health = CCSP_COMMON_COMPONENT_HEALTH_Yellow;
    ...
    returnStatus =
        pDslhCpeController->RegisterCcspDataModel
            (
                ...
                CCSP_COMPONENT_NAME_MOTIONDETECTION,            /* Component Name    */
                CCSP_COMPONENT_VERSION_MOTIONDETECTION,         /* Component Version */
                CCSP_COMPONENT_PATH_MOTIONDETECTION,            /* Component Path    */
    ...
    if ( returnStatus == ANSC_STATUS_SUCCESS || returnStatus == CCSP_SUCCESS)
    {
        ...
        //system("touch > /tmp/ssp_engage");
        g_pComponent_COMMON_MotionDetection->Health = CCSP_COMMON_COMPONENT_HEALTH_Green;
    }
    ...
    )
{
    ...
    if(  g_pComponent_COMMON_MotionDetection == NULL)
    {
        return ANSC_STATUS_SUCCESS;
    }
    ...
    {
        _ansc_sprintf(CrName, "%s%s", g_Subsystem, CCSP_DBUS_INTERFACE_CR);
        _ansc_sprintf(CpName, "%s%s", g_Subsystem, CCSP_COMPONENT_NAME_MOTIONDETECTION);
    }
    else
    {
        _ansc_sprintf(CrName, "%s", CCSP_DBUS_INTERFACE_CR);
        _ansc_sprintf(CpName, "%s", CCSP_COMPONENT_NAME_MOTIONDETECTION);
    }
    ...
    if (  g_pComponent_COMMON_MotionDetection ) AnscFreeMemory( g_pComponent_COMMON_MotionDetection);

     g_pComponent_COMMON_MotionDetection = NULL;
    pSsdCcdIf                = NULL;
...
{
    return  g_pComponent_COMMON_MotionDetection->Name;
}
...
{
    return  g_pComponent_COMMON_MotionDetection->Version;
}
...
{
    return  g_pComponent_COMMON_MotionDetection->Author;
}
...
{
    return  g_pComponent_COMMON_MotionDetection->Health;
}
...
{
    return  g_pComponent_COMMON_MotionDetection->State;
}
...
{
    return  g_pComponent_COMMON_MotionDetection->LogEnable;
}
...
{
    if( g_pComponent_COMMON_MotionDetection->LogEnable == bEnabled) return ANSC_STATUS_SUCCESS;
     g_pComponent_COMMON_MotionDetection->LogEnable = bEnabled;
    if(bEnabled) g_iTraceLevel = (INT)  g_pComponent_COMMON_MotionDetection->LogLevel;
...
{
    return  g_pComponent_COMMON_MotionDetection->LogLevel;
}
...
{
    if( g_pComponent_COMMON_MotionDetection->LogLevel == LogLevel) return ANSC_STATUS_SUCCESS;
     g_pComponent_COMMON_MotionDetection->LogLevel = LogLevel;
    if( g_pComponent_COMMON_MotionDetection->LogEnable) g_iTraceLevel = (INT)  g_pComponent_COMMON_MotionDetection->LogLevel;
...
{
    return  g_pComponent_COMMON_MotionDetection->MemMinUsage;
}
    ...
    )
{
    ...
    size = AnscGetComponentMemorySize(CCSP_COMPONENT_NAME_MOTIONDETECTION);
    if (size == -1 )
        size = 0;
    ...
    )
{
    ...
    /* AnscSetTraceLevel((INT) g_pComponent_COMMON_MotionDetection->LogLevel); */
